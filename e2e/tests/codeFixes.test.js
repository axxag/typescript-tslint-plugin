...
const mockFileName = path
  .join(__dirname, "..", "project-fixture", "main.ts")
  .replace(/\\/g, "/");
...
describe("CodeFixes", () => {
  ...
  it("should return fix and disables for single error", async () => {
    ...
    await server.close();
    const codeFixesResponse = await getFirstResponseOfType(
      "getCodeFixes",
      server
    );
    ...
    assert.deepEqual(codeFixesResponse.body, [
      {
        fixName: "eslint:array-type",
        description:
          "Fix: Array type using 'Array<T>' is forbidden for simple types. Use 'T[]' instead.",
        changes: [
          {
      ...
      {
        fixName: "eslint:disable:array-type",
            ...
            textChanges: [
              {
                ...
                newText: "// eslint:disable-next-line: array-type\n",
              },
            ],
          },
        ],
  ...
  it("should return individual fixes and fix all for multiple errors of same type in file", async () => {
    ...
    await getCombinedCodeFixes(server, "eslint:array-type");
    ...
    const combinedFixesResponse = await getFirstResponseOfType(
      "getCombinedCodeFix",
      server
    );
  ...
  it("should not return ts-lint fixes on non-eslint errors", async () => {
    server = createServerForFile(`const a = 1; a = 2`);
    await getCodeFixes(server, {
      startLine: 1,
      startOffset: 13,
      endLine: 1,
  ...
  it("should not return TS Lint fix-all for non-fixable errors", async () => {
    server = createServerForFile(`const foo = 123; food`);
    await getCodeFixes(server, {
      startLine: 1,
      startOffset: 18,
      endLine: 1,
